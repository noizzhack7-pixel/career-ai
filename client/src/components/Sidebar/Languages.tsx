...
export const Languages = ({ employeeData }: LanguagesProps) => {
  return (
    <section id="languages-section" className="bg-white p-6 rounded-card shadow-card">
      ...
      <div className="space-y-3">
        {employeeData?.languages &&
          employeeData.languages.split(',').map((language: string, index: number) => (
            <div key={index} className="flex items-center justify-between">
              <span className="font-semibold text-neutral-dark">{language.trim()}</span>
              {index === 0 ? (
