...
export const MatchingCTA: React.FC<MatchingCTAProps> = ({ onRunMatch }) => {
  ...
  const handleStartProcess = (e?: React.MouseEvent) => {
    ...
    setCurrentStage(0);
    onRunMatch?.(); // Call immediately when clicked
  };

  useEffect(() => {
    if (!isProcessing) return;

    const stageDuration = 1100; // 1.5 seconds per stage

    const timer = setTimeout(() => {
      if (currentStage < PROCESS_STAGES.length - 1) {
        setCurrentStage(prev => prev + 1);
      } else {
        // Complete the process - just reset the state
        setTimeout(() => {
          setIsProcessing(false);
        }, 400); // Small delay after last stage
      }
    }, stageDuration);
