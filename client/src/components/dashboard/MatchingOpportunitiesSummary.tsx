...
export const MatchingOpportunitiesSummary: React.FC<MatchingOpportunitiesSummaryProps> = ({
  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      { text: 'מחשב אחוזי התאמה...', delay: 2000 },
                ...
                .map((position: any, index: number) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-bold text-lg text-primary min-h-[56px] line-clamp-2">{position.position_name}</h3>
                      <div className="shrink-0">
                        <MatchScore score={Math.floor(position.score)} compact={true} showScore={true} />
                      </div>
                    </div>
                    <h3 className="mb-2 font-small text-base text-neutral-dark min-h-[56px] line-clamp-2">{position.profile_name}</h3>
                    ...
                    <button style={{ borderRadius: '12px' }} className="w-full bg-primary text-white py-3 b rounded-button text-sm font-semibold hover:bg-primary-dark transition-colors h-[44px] flex items-center justify-center gap-2 mt-auto">
                      פרטי משרה
                    </button>
                  </motion.div>
                ))}
